    ...
    <div>
        <section class="grid grid-cols-1 xl:grid-flow-col gap-6">
            <div class="row-span-3 bg-white shadow rounded-lg">
                ...
                <div class="p-4 -mb-2 py-4 flex flex-wrap flex-grow justify-between">
                        ...
                        <div class="relative inline-block text-left">
                            ...
                            <transition enter-active-class="transition ease-out duration-100"
                                ...
                                <div v-if="open"
                                    ...
                                    <input v-model="store.searchQuery" @input="store.debouncedFetch"
                                        placeholder="Search products..."
                                        class="w-full mb-3 px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500" />

                                    <select v-model="store.isActive" @change="store.debouncedFetch"
                                        class="w-full mb-3 px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500">
                                    ...
                                    <div class="grid grid-cols-2 gap-3 mb-3">
                                        <input type="number" v-model.number="store.minPrice"
                                            @input="store.debouncedFetch" placeholder="Min Price"
                                            class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500" />

                                        <input type="number" v-model.number="store.maxPrice"
                                            @input="store.debouncedFetch" placeholder="Max Price"
                                            class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500" />
                                    </div>

                                    <div class="grid grid-cols-2 gap-3">
                                        <input type="number" v-model.number="store.minStock"
                                            @input="store.debouncedFetch" placeholder="Min Stock"
                                            class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500" />

                                        <input type="number" v-model.number="store.maxStock"
                                            @input="store.debouncedFetch" placeholder="Max Stock"
                                            class="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500" />
                                    </div>
                        ...
                        <table class="min-w-full">
                            ...
                            <tbody class="bg-white">
                                <tr v-for="product in store.products" :key="product.id">
                                    ...
                                    <td class="px-6 py-4 border-b border-gray-200">{{ priceFormatter(product.price) }}
                                    </td>
                                    <td class="px-6 py-4 border-b border-gray-200">
                                        <span
                                        ...
                                        <span
                                            v-else
                                            class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Not Active</span>
                                    </td>
                                   
                                    <td class="px-6 py-4 text-right border-b border-gray-200 text-sm font-medium">
                                        ...
                                        <button @click="deleteModal(product)"
        ...
        <Modal :show="showModal" :title="isEditing ? 'Edit Product' : 'Add Product'" @close="showModal = false">
            <form @submit.prevent="handleSubmit" class="max-w-lg mx-auto p-6 bg-white rounded shadow space-y-6">
                <div>
